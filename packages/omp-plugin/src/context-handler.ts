...
function collectMessageEntryIds(
	...
	if (ids.length !== expectedLength) {
		...
		log(
			`[magic-context][omp]${sessionId ? `[${sessionId}]` : ""} collectMessageEntryIds length mismatch: ` +
...
export function collectMessageEntryIdsByRef(
	...
	if (resolved < messages.length) {
		log(
			`[magic-context][omp]${sessionId ? `[${sessionId}]` : ""} ` +
...
export function registerPiContextHandler(
	...
	pi.on("context", async (event, ctx) => {
		...
		try {
			...
			if (sessionId === undefined) {
				...
				log(
					"[magic-context][omp] context event fired with no session id (falling through unmodified)",
				);
				return;
			}
			...
			log(
				`[magic-context][omp] context handler failed (continuing without mutation): ${message}`,
				stack,
			);
	...
	log(
		"[magic-context][omp] registered context handler (tagging + drops + nudges)",
	);
}
...
function maybeFireHistorian(args: {
	...
	try {
		...
		if (
			...
			isSaneLimit(ctx.model?.contextWindow ?? undefined)
		) {
