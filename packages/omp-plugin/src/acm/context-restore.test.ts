...
import { z } from "zod";
import registerACMExtension from "./tools.js";
...
describe("ACM restored-session sanitation", () => {
	it("removes a persisted travel result after session_start clears pending state", async () => {
		...
		const messages = [
			{ role: "branchSummary", summary: "Continue from the handoff branch" },
			{
				role: "toolResult",
		...
		expect(await context?.({ messages }, ctx)).toEqual({
			messages: [
				{ role: "branchSummary", summary: "Continue from the handoff branch" },
				{ role: "user", content: text("new request after restore") },
			],
	...
	it("does not override an unchanged outbound request", async () => {
		...
		expect(
			await context?.({ messages }, { sessionManager: {} }),
		).toBeUndefined();
